...
- 已完成 `openpi_rtc` 数值权重预检：`pi05_base` 12.44GB 参数能接入 RTC 模型结构，51 个实际注入 leaf、过滤 2 个 `ShapeDtypeStruct` knob leaf；全量 grad 在 24GB 4090 当前占用下 OOM，见 `reports/openpi_rtc_numeric_weight_preflight.md` 和 `reports/openpi_rtc_numeric_grad_attempt.md`。
...
- `scripts/run_openpi_rtc_numeric_dry_run.py`：分阶段运行 `openpi_rtc` 数值 dry-run，支持权重预检、forward 和 grad 模式。
...
1. 先释放或隔离非 RoboChallenge GPU 进程，再重跑 `forward` 数值 loss；当前全量 grad 在 24GB 4090 上已记录 OOM。
2. 将下一步训练 dry-run 改为 LoRA/冻结层反向或更小模型状态，先拿到 1-step grad/checkpoint，再考虑长训。
