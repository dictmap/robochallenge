...
- 审计状态：`passed=True`。
- 当前是否已有 `demo.py --checkpoint` 可直接消费的 LoRA 完整 checkpoint：`True`。
- 本次已显式写出完整 12GB+ checkpoint，并完成恢复 smoke。
- 如需重建完整 checkpoint，可重新运行 `python3 scripts/audit_openpi_rtc_lora_inference_checkpoint_layout.py --materialize --force`。
...
- 物化执行：`attempted=True`，`passed=True`。
